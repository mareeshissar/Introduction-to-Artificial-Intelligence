	...
	for (int i = 0; i < num_iter; i++)
	{
		...
		while (target_found == false)
		{
			...
			else
			{
				...
				temp_cell = map1.find_next_cell_rule_distance(current_cell,k_new);
				p1 = temp_cell.ReturnY();
				p2 = temp_cell.ReturnX();
			...
			point p_target = map1.return_target();
			...
			k_new = p_target.Return_terrain();
			while (k_new == p_target.Return_terrain())
			{
				k_new = 0 + rand() % 4;                 //randomly send one of them as the value of terraing not there
			}
			map1.update_belief_rule_1(p1, p2);
		...
		while (target_found == false)
		{
			...
			else
			{
				...
				temp_cell = map2.find_next_cell_rule_1(current_cell,k_new);
				p1 = temp_cell.ReturnY();
				p2 = temp_cell.ReturnX();
			...
			point p_target = map2.return_target();
