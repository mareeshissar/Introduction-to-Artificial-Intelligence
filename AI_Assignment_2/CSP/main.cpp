...

{
    ...
    int flaggedCSPMines_avg = 0;
    int simul = 1;
    cout << "Number of safe places at begin:"<< (d*d) - n <<endl;

	for (int iter = 0; iter < simul; iter++)
	{ 
		int baseline = 0;
		int CSP = 0;
...
*/
    ...
    //*******************************************************************APPLYING CSP*************************************************************
							...
							if ((8 - totalNumberOfMines) - numberOfRevealedSafe == numberOfHiddenNeighbors)
							{
								for (int i = 0; i < neighborsWithinCSP.size(); i++)
								{
									if (b1.IsVisited(neighborsWithinCSP[i].ReturnY(), neighborsWithinCSP[i].ReturnX()) == false)
									{
										...
										inference_queue.push_back(neighborsWithinCSP[i]);
                                        
                                        //adding safe points to the queue.
									}
								}
							}
						}
            ...
            flaggedCSPMines_avg = flaggedCSPMines_avg + CSP;

		}
		cout << "number of flagged mines totally- " << flaggedMines << endl;
		cout << "number of flagged CSP mines totally - " << CSP << endl;
        cout << "number of flagged baseline mines totally - " << baseline << endl;
        ...
        //cout << "Avg number of CSP flagged mines- " << flaggedCSPMines_avg / simul <<endl;
        cout<<"--------------------------------------------END--------------------------------------------------------------------------"<<endl;
        ...
        for(int i=0;i<d;i++)
        {
            for(int j=0; j<d;j++)
            {
                if(b1.GetValue(i,j) != 10 && b1.GetValue(i,j) != 20)
                {
                    number_safe = number_safe + 1;
                }
            }
        }
        cout << "Number of safe places:"<<number_safe<<endl;
	}
